...
let model_converter_test (ctx : context) =
    ...
    Goal.add g4
      [
        mk_eq ctx yr (Arithmetic.mk_add ctx [ xr; Real.mk_numeral_nd ctx 1 1 ]);
      ];
    Goal.add g4 [ mk_gt ctx yr (Real.mk_numeral_nd ctx 1 1) ];
    (let ar = Tactic.apply (mk_tactic ctx "simplify") g4 None in
     if
       get_num_subgoals ar == 1
       && ( is_decided_sat (get_subgoal ar 0)
          || is_decided_unsat (get_subgoal ar 0) )
     then raise (TestFailedException "")
     else Printf.printf "Test passed.\n");
    let ar =
        Tactic.apply
          (and_then ctx (mk_tactic ctx "simplify")
    ...
    if q != SATISFIABLE then raise (TestFailedException "")
    else
...
let basic_tests (ctx : context) =
    ...
    let fapp =
        mk_app ctx f [ Expr.mk_const ctx x bs; Expr.mk_const ctx y bs ]
    in
    (* Z3.Expr.mk_fresh_const: Creates a fresh constant with a name prefixed with a string. *)
    let fargs2 = [ mk_fresh_const ctx "cp" bs ] in
    let domain2 = [ bs ] in
    let fapp2 = mk_app ctx (mk_fresh_func_decl ctx "fp" domain2 bs) fargs2 in
    (* Boolean.mk_eq: Creates the equality between two expr's. *)
    let trivial_eq = mk_eq ctx fapp fapp in
    let nontrivial_eq = mk_eq ctx fapp fapp2 in
    (* mk_goal: context -> models -> unsat_cores -> proofs -> goal *)
    ...
    (* If proofs is true, then proof generation is enabled for the new goal.
       Remark, the Z3 context c must have been created with proof generation support if it's set to true here *)
    let g = mk_goal ctx true false false in
    Goal.add g [ trivial_eq ];
    Goal.add g [ nontrivial_eq ];
    Printf.printf "%s\n" ("Goal: " ^ Goal.to_string g);
    (* i guess if `Some symbol` is passed, its the same as mk_solver_for_logic
       mk_solver_for_logic: Create a new solver customized for the given logic. It behaves like Z3_mk_solver if the logic is unknown or unsupported. *)
    (let solver = mk_solver ctx None in
     (* Solver.add: Assert a constraint (or multiple) into the solver. *)
     List.iter (fun a -> Solver.add solver [ a ]) (get_formulas g);
     (* check: Checks whether the assertions in the solver are consistent or not. *)
     if check solver [] != SATISFIABLE then raise (TestFailedException "")
     else Printf.printf "Test passed.\n");
    (* mk_tactic: Return a tactic associated with the given name *)
    (* apply : tactic -> Goal.goal -> Params.params option -> ApplyResult.apply_result *)
    (* A Params objects represents a configuration in the form of Symbol.symbol/value pairs. *)
    (* apply_result: contains the subgoals that were produced. *)
    ...
    (let ar = Tactic.apply (mk_tactic ctx "smt") g None in
     if get_num_subgoals ar == 1 && not (is_decided_sat (get_subgoal ar 0)) then
       raise (TestFailedException "")
     else Printf.printf "Test passed.\n");
    (* mk_sort: create a new bit-vector sort *)
    Goal.add g
      [
        mk_eq ctx
    ...
    (let ar = Tactic.apply (mk_tactic ctx "smt") g None in
     if get_num_subgoals ar == 1 && not (is_decided_unsat (get_subgoal ar 0))
     then raise (TestFailedException "")
     else Printf.printf "Test passed.\n");
    ...
    (let g3 = mk_goal ctx true true false in
     let xc =
         Expr.mk_const ctx (Symbol.mk_string ctx "x") (Integer.mk_sort ctx)
     in
     let yc =
         Expr.mk_const ctx (Symbol.mk_string ctx "y") (Integer.mk_sort ctx)
     in
    ...
    Printf.printf "Numerator: %s Denominator: %s\n"
      (Real.numeral_to_string inum)
      (Real.numeral_to_string iden);
    if
      Real.numeral_to_string inum <> "42" || Real.numeral_to_string iden <> "43"
    then raise (TestFailedException "")
    else Printf.printf "Test passed.\n";
    if to_decimal_string rn 3 <> "0.976?" then raise (TestFailedException "")
    else Printf.printf "Test passed.\n";
    if
      to_decimal_string (Real.mk_numeral_s ctx "-1231231232/234234333") 5
      <> "-5.25640?"
    then raise (TestFailedException "")
    else if
      to_decimal_string
        (Real.mk_numeral_s ctx "-123123234234234234231232/234234333")
        5
      <> "-525641278361333.28170?"
    then raise (TestFailedException "")
    else if
      to_decimal_string (Real.mk_numeral_s ctx "-234234333") 5 <> "-234234333"
    then raise (TestFailedException "")
    else if
      to_decimal_string (Real.mk_numeral_s ctx "234234333/2") 5 <> "117117166.5"
    then raise (TestFailedException "");
    (* Error handling test. *)
    try
    ...
    with Z3.Error _ -> Printf.printf "Exception caught, OK.\n"
...
let quantifier_example1 (ctx : context) =
    ...
    in
    (* mk_bound: context -> de-bruijin index -> sort of the bound variable -> expr
       Create a bound variable. Bound variables are indexed by de-Bruijn indices. *)
    let vars =
        [
    ...
    (* Integer.mk_numeral_i: returns A Term with the given value and sort Integer *)
    let body_vars =
        Boolean.mk_and ctx
          [
            mk_eq ctx
...
let fpa_example (ctx : context) =
    ...
    let args2 =
        [
          c2;
          Boolean.mk_not ctx (Boolean.mk_eq ctx rm (RoundingMode.mk_rtz ctx));
        ]
    in
    let c3 = Boolean.mk_and ctx args2 in
    let and_args =
        [
          Boolean.mk_not ctx (mk_is_zero ctx y);
          Boolean.mk_not ctx (mk_is_nan ctx y);
          Boolean.mk_not ctx (mk_is_infinite ctx y);
        ]
    in
    ...
    let c2 =
        mk_to_fp_bv ctx
          (mk_numeral_string ctx "4619567317775286272"
             (BitVector.mk_sort ctx 64))
          (mk_sort ctx 11 53)
    in
    let c3 =
        mk_to_fp_int_real ctx (RoundingMode.mk_rtz ctx)
          (mk_numeral_string ctx "2" (Integer.mk_sort ctx))
          (mk_numeral_string ctx "1.75" (Real.mk_sort ctx))
          (FloatingPoint.mk_sort ctx 11 53)
    in
    let c4 =
        mk_to_fp_real ctx (RoundingMode.mk_rtz ctx)
          (mk_numeral_string ctx "7.0" (Real.mk_sort ctx))
          (FloatingPoint.mk_sort ctx 11 53)
    in
    let args3 = [ mk_eq ctx c1 c2; mk_eq ctx c1 c3; mk_eq ctx c1 c4 ] in
